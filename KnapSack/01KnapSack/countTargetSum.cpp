// Problem statement: Given a array, put '+' or '-' before each
// number such that the resultant of all the numbers is equal 
// to the given sum. Count all such combinations present.
...
public:
   int memo[10002][102];            // The array size causes problem
    ...
    int topDown(int N, int arr[], int sum){
     ...
     int targetSum(int N, int arr[], int targetSum)
{
          // code here
          int sum = accumulate(arr,arr+N,0);
		  int reqSum = (sum - targetSum)/2;
//           Recursion
//           return recursion(N,arr,reqSum);
//           Memoization 
//           memset(memo,-1,sizeof(memo));
//           return memoization(N,arr,sum/2);
//           bottomUp
//           return topDown(N,arr,reqSum);
     }
};
...
int main(){
    ...
    while(t--){
        ...
        Solution ob;
        cout<<ob.countDiffSubset(N, arr,diff)<<endl;
    }
    return 0;
}  // } Driver Code Ends
